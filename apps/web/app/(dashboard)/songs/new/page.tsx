...
export default function NewSongPage() {
  ...
  useEffect(() => {
    artistsApi.list().then((p) => setArtists(p.items)).catch(() => {});
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 503) {
        setError(
          "El storage no está configurado o el bucket no permite subir. " +
            "Verificá las env vars R2_* y el CORS del bucket."
        );
      } else {
